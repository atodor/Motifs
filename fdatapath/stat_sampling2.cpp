 ...
 int rc;
  ...
  if (sampling)
	...
	selected_samples[i] = i;
      if(par.motifs.size()>N_SAMPLES)
	{
	  for (int i=n_samples; i < par.motifs.size(); i++) 
	    {
	      int j = rand() % (i + 1);   // important: inclusive range
	      if (j < n_samples)
		selected_samples[j] = i;
	    }
	}
    }
  else
  ...
  double f = (double)par.motifs.size() / (double)n_samples;
